...
bestWorstDelay = inf; % Initialize the best worst round-trip delay
bestCombination = [];
...
for combIdx = 1:size(allCombinations, 1)
    ...
    % Compute the worst round-trip delay for the current combination
    ...
    if worstAnycastDelay < bestWorstDelay
        bestWorstDelay = worstAnycastDelay;
...
bestWorstLoad = max(max(Loads(:, 3:4))); % Worst link load for the best combination
